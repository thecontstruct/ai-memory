#!/usr/bin/env python3
"""Gemini CLI PreCompress adapter — saves session summary before context compression.

Architecture: §2 Data Flow Capture Path (session summary)
PRD: FR-203

Gemini PreCompress maps to canonical PreCompact. Captures session summary
to discussions collection before Gemini compresses context.
"""
...
logger = logging.getLogger("ai_memory.adapters.gemini.pre_compress")
...
def main() -> int:
    ...
    try:
        pipeline_script = os.path.join(
            INSTALL_DIR, "adapters", "pipeline", "store_async.py"
        )
        fork_to_background(event, pipeline_script)
        logger.info(
            "pre_compress_complete",
            extra={
