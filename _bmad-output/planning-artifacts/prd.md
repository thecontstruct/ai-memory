...
- Detection method: check for `gemini` binary in PATH for Gemini CLI; check for `agent` or `cursor-agent` binary in PATH for Cursor; check for `codex` binary in PATH for Codex CLI.
