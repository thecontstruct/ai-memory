...
Global constraints (GC-1 through GC-12) are always active. The constraints below apply only during WF-EXECUTION. When Execution exits (to Planning or Integration), these constraints are dropped.
...
| ID | Name | Severity |
|----|------|----------|
| EC-01 | MUST Verify Story Requirements Against Current Project Files Before Dispatch | CRITICAL |
| EC-02 | MUST Use Implementation Instruction Template for Every DEV Dispatch | CRITICAL |
...
| EC-07 | Implementation Decisions Made by DEV Must Be Reviewed and Documented | MEDIUM |
...
- EC-02: Did I use the implementation instruction template for this dispatch?
...
- EC-06: Did I verify DEV's completion claim independently?
- EC-07: Have DEV implementation decisions been reviewed and documented?
...
|---|---|---|
| EC-01: Story dispatched without verification | CRITICAL | Stop execution, verify story, update if needed |
| EC-02: DEV dispatched without instruction template | CRITICAL | Recall, build proper instruction, redispatch |
...
| EC-06: DEV self-certification accepted without verification | CRITICAL | Run full verification cycle regardless |
