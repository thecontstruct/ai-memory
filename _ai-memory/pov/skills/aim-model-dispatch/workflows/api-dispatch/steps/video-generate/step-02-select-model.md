...
description: 'Present video-generate model options to user'
...
## STEP GOAL
Choose model for video-generate. If user did not specify, present options and wait for confirmation.
...
python3 "${SKILL_DIR}/scripts/openrouter-api/list-models.py" \
  --category video-gen --query "${INPUT_SOURCE}" --limit 10
```
...
| Model ID | Notes |
|---|---|
| `runway/gen-4-turbo` | Best quality, fastest Runway model (recommended) |
| `runway/gen-3-alpha` | High quality, slower |
| `kling-ai/kling-v2-master` | Strong realistic motion |
| `kling-ai/kling-v1-5` | Faster, lower cost |
| `pika/pika-2.2` | Good for stylized content |
| `minimax/video-01` | Strong motion coherence |
| `luma/dream-machine` | Smooth camera motion |

### 3. Wait for User Model Choice

> "Recommended: **runway/gen-4-turbo**. Accept this model, or pick a different one?"
