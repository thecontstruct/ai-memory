---
name: 'step-v-01-validate-maintenance'
description: 'Validation check: Verify maintenance workflow output against checklist criteria'
---

# Validate Step 1: Validate Maintenance Phase Output

## STEP GOAL:

Validate that the maintenance workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The maintenance workflow produces a tightly scoped fix that has been triaged, classified, implemented by DEV, reviewed with zero legitimate issues, and results in an updated CHANGELOG.md — with user approval and without scope expansion.
...
- 🎯 Validate maintenance output against checklist criteria
...
- Available context: maintenance workflow output (triage record, fix implementation, review results, CHANGELOG.md update) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: maintenance create mode (steps-c/) must be complete; fix must be implemented, reviewed, and CHANGELOG.md updated
...
Identify and read the output artifacts produced by the maintenance workflow's create mode (steps-c/). This includes:
- Issue triage record (severity classification, root cause if applicable)
- Classification result: maintenance fix vs. new feature
- Analyst diagnosis record (if applicable for complex bugs)
- Maintenance task record
- Implemented fix (scoped to the issue, no scope expansion)
- Review cycle result (zero legitimate issues)
