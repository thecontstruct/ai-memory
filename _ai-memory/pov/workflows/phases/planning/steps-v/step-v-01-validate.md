---
name: 'step-v-01-validate-planning'
description: 'Validation check: Verify planning workflow output against checklist criteria'
---

# Validate Step 1: Validate Sprint Planning Output

## STEP GOAL:

Validate that the planning workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The planning workflow produces an approved sprint plan with implementation-ready story files — each story detailed enough for a DEV agent to implement without guessing, with user approval confirming the sprint is ready for execution.
...
- 🎯 Validate planning output against checklist criteria
...
- Available context: planning workflow output (sprint plan, story files, sprint-status.md) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: planning create mode (steps-c/) must be complete; sprint plan and story files must exist with user approval
...
Identify and read the output artifacts produced by the planning workflow's create mode (steps-c/). This includes:
- Sprint plan document (sprint-status.md or equivalent)
- Individual story files for each sprint story
- Evidence of Parzival's sprint plan review
- Retrospective completion for subsequent sprints (if applicable)
- Evidence that stories are implementation-ready (specific acceptance criteria, no implementation decisions left to DEV)
- User approval of the sprint plan
