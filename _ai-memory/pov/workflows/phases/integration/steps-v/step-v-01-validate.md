---
name: 'step-v-01-validate-integration'
description: 'Validation check: Verify integration workflow output against checklist criteria'
---

# Validate Step 1: Validate Integration Phase Output

## STEP GOAL:

Validate that the integration workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The integration workflow produces a full integration test pass result with zero legitimate issues remaining, Architect cohesion confirmation, and explicit user approval — verifying that the complete feature set works together coherently.
...
- 🎯 Validate integration output against checklist criteria
...
- Available context: integration workflow output (integration test plan, DEV full review results, Architect cohesion check, fix cycle results, final verification) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: integration create mode (steps-c/) must be complete; all integration test results and approval must exist
...
Identify and read the output artifacts produced by the integration workflow's create mode (steps-c/). This includes:
- Integration scope definition
- Integration test plan (complete, not spot-check)
- DEV full review pass results
- Architect cohesion check results
- Parzival's findings review
- Fix cycle completion record (all legitimate issues resolved)
- Final verification pass result (zero issues)
