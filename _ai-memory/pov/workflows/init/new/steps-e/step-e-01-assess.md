---
name: 'step-e-01-assess-init-new'
description: 'Edit mode: Assess init-new output and propose changes'
nextStepFile: './step-e-02-apply-edit.md'
---

# Edit Step 1: Assess Init New Output

## STEP GOAL:

Assess the current state of init-new output, identify what needs to change, and propose specific edits for user approval.
...
- 🎯 Assess init-new output for issues identified in validation
...
- Available context: init-new output files, validation report (if available)
- Focus: Assessment and proposal only — no modifications
- Limits: Do not apply changes, only propose them
- Dependencies: Validation report from init-new validation, or user-identified issues that drive this edit

## EDIT ASSESSMENT SEQUENCE

### 1. Load Current Output

Read the init-new output files that need editing. If a validation report exists, load it to understand what failed.
