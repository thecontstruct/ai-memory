---
name: 'step-v-01-validate-session-status'
description: 'Validation check: Verify session-status workflow output against checklist criteria'
---

# Validate Step 1: Validate Session Status Output

## STEP GOAL:

Validate that the session-status workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The session-status workflow produces a concise read-only status snapshot covering current sprint, active tasks, blockers, risks, and last session summary — sourced directly from tracking files without modification.
...
- 🎯 Validate session-status output against checklist criteria
...
- Available context: session-status workflow output (Quick Status report) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: session-status create mode (steps-c/) must be complete; tracking files must exist
...
Identify and read the output artifacts produced by the session-status workflow's create mode (steps-c/). This includes:
- The Quick Status report presented to the user
- Evidence that SESSION_WORK_INDEX.md, task-tracker.md, blockers-log.md, and risk-register.md were read
- Confirmation that no files were modified during the status check
