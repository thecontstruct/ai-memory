---
name: 'step-v-01-validate-session-blocker'
description: 'Validation check: Verify session-blocker workflow output against checklist criteria'
---

# Validate Step 1: Validate Session Blocker Output

## STEP GOAL:

Validate that the session-blocker workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The session-blocker workflow produces a logged blocker entry in the blockers tracking file, including a precise blocker description, root cause analysis, and at least two resolution options.
...
- 🎯 Validate session-blocker output against checklist criteria
...
- Available context: session-blocker workflow output (blocker log entry, resolution options) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: session-blocker create mode (steps-c/) must be complete; blockers-log.md must have the new entry
...
Identify and read the output artifacts produced by the session-blocker workflow's create mode (steps-c/). This includes:
- The new blocker entry in the blockers-log.md tracking file
- The blocker analysis presented to the user (root cause, resolution options)
- Evidence that prior issues were checked before analysis (GC-14 compliance)
