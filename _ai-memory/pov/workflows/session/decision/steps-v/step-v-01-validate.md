---
name: 'step-v-01-validate-session-decision'
description: 'Validation check: Verify session-decision workflow output against checklist criteria'
---

# Validate Step 1: Validate Session Decision Output

## STEP GOAL:

Validate that the session-decision workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The session-decision workflow produces a structured decision log entry containing the decision context, all options with tradeoffs, Parzival's recommendation, and the user's recorded choice.
...
- 🎯 Validate session-decision output against checklist criteria
...
- Available context: session-decision workflow output (decision log entry, options presented, outcome recorded) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: session-decision create mode (steps-c/) must be complete; decision log must contain the new entry
...
Identify and read the output artifacts produced by the session-decision workflow's create mode (steps-c/). This includes:
- The structured decision presentation (context, options, tradeoffs, recommendation)
- The decision log entry recording the user's choice — located at `{oversight_path}/tracking/decision-log.md`
- Evidence that at least two options were presented and no tradeoffs were hidden
