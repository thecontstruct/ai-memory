---
name: 'step-v-01-validate-session-close'
description: 'Validation check: Verify session-close workflow output against checklist criteria'
---

# Validate Step 1: Validate Session Close Output

## STEP GOAL:

Validate that the session-close workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The session-close workflow produces a complete session closeout package: session summary, updated tracking files (task-tracker.md, blockers-log.md, risk-register.md, SESSION_WORK_INDEX.md), a handoff document, and a Qdrant save (or logged graceful degradation if unavailable).
...
- 🎯 Validate session-close output against checklist criteria
...
- Available context: session-close workflow output (handoff document, updated tracking files, session summary) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: session-close create mode (steps-c/) must be complete; all tracking files and handoff document must exist
...
Identify and read the output artifacts produced by the session-close workflow's create mode (steps-c/). This includes:
- The SESSION_HANDOFF_*.md document in session-logs/
- Updated tracking files: task-tracker.md, blockers-log.md, risk-register.md, SESSION_WORK_INDEX.md
- Session summary content
- Qdrant save result or graceful degradation log entry
