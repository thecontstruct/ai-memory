---
name: 'step-e-01-assess-session-start'
description: 'Edit mode: Assess session-start output and propose changes'
nextStepFile: './step-e-02-apply-edit.md'
---

# Edit Step 1: Assess Session Start Output

## STEP GOAL:

Assess the current state of session-start output, identify what needs to change, and propose specific edits for user approval.
...
- 🎯 Assess session-start output for issues identified in validation
...
- Available context: session-start output files, validation report (if available)
- Focus: Assessment and proposal only — no modifications
- Limits: Do not apply changes, only propose them
- Dependencies: Validation report from session-start validation, or user-identified issues that drive this edit

## EDIT ASSESSMENT SEQUENCE

### 1. Load Current Output

Read the session-start output files that need editing. If a validation report exists, load it to understand what failed.
...
**Proposed Changes for Session Start:**
