---
name: 'step-v-01-validate-cycles-review-cycle'
description: 'Validation check: Verify review-cycle workflow output against checklist criteria'
---

# Validate Step 1: Validate Review Cycle Output

## STEP GOAL:

Validate that the review-cycle workflow produced correct and complete output by checking against the workflow's checklist.md criteria. The review-cycle workflow produces a zero-legitimate-issues review result — reached through iterative DEV dispatch and code review passes, where each issue was addressed before the cycle closed.
...
- 🎯 Validate review-cycle output against checklist criteria
...
- Available context: review-cycle workflow output (review pass results, issue resolution record, final zero-issue pass) and checklist.md
- Focus: Validation only — no corrections
- Limits: Do not fix issues, only report them
- Dependencies: review-cycle create mode (steps-c/) must be complete; final review pass must show zero legitimate issues
...
Identify and read the output artifacts produced by the review-cycle workflow's create mode (steps-c/). This includes:
- Initial completeness verification result
- Review pass results (all passes, not just the last one)
- Issue correction instructions sent to DEV (all issues addressed, not partial)
- Evidence that the review was run on all changed code, not just new code
- Final review pass confirming zero legitimate issues
- Evidence that pre-existing issues were not treated as out of scope
