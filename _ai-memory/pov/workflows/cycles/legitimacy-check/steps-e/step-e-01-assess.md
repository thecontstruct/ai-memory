---
name: 'step-e-01-assess-cycles-legitimacy-check'
description: 'Edit mode: Assess legitimacy-check output and propose changes'
nextStepFile: './step-e-02-apply-edit.md'
---

# Edit Step 1: Assess Legitimacy Check Output

## STEP GOAL:

Assess the current state of legitimacy-check output, identify what needs to change, and propose specific edits for user approval.
...
- 🎯 Assess legitimacy-check output for issues identified in validation
...
- Available context: legitimacy-check output files, validation report (if available)
- Focus: Assessment and proposal only — no modifications
- Limits: Do not apply changes, only propose them
- Dependencies: Validation report from cycles-legitimacy-check validation, or user-identified issues that drive this edit

## EDIT ASSESSMENT SEQUENCE

### 1. Load Current Output

Read the legitimacy-check output files that need editing. If a validation report exists, load it to understand what failed.
...
**Proposed Changes for Legitimacy Check:**
