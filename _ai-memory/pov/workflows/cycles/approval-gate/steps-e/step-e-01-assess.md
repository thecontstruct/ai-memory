---
name: 'step-e-01-assess-cycles-approval-gate'
description: 'Edit mode: Assess approval-gate output and propose changes'
nextStepFile: './step-e-02-apply-edit.md'
---

# Edit Step 1: Assess Approval Gate Output

## STEP GOAL:

Assess the current state of approval-gate output, identify what needs to change, and propose specific edits for user approval.
...
- 🎯 Assess approval-gate output for issues identified in validation
...
- Available context: approval-gate output files, validation report (if available)
- Focus: Assessment and proposal only — no modifications
- Limits: Do not apply changes, only propose them
- Dependencies: Validation report from cycles-approval-gate validation, or user-identified issues that drive this edit

## EDIT ASSESSMENT SEQUENCE

### 1. Load Current Output

Read the approval-gate output files that need editing. If a validation report exists, load it to understand what failed.
...
**Proposed Changes for Approval Gate:**
