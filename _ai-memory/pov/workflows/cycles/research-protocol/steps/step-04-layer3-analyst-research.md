...
- Limits: Analyst researches the codebase only. This is not a general knowledge query. Both Layer 1 and Layer 2 must be exhausted before activating Analyst.
...
ONLY when Layer 3 research is complete and results evaluated, either return to calling workflow (if answer found) or load and read fully {nextStepFile} for user escalation.
